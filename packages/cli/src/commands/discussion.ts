...
export const discussionCommand = new Command("discussion")
	.description("Show Discussion context")
	.argument("<ref>", "Discussion reference (owner/repo#number)")
	...
	.action(async (refStr: string, opts: { format: string; cache: boolean }) => {
		...
		try {
			...
			const ctx = await client.getDiscussion(ref);
			const summary = summarizeDiscussion(ctx);

			const formatters: Record<string, () => string> = {
				table: () => formatDiscussion(ctx, summary),
				json: () => formatDiscussionJson(ctx),
				minimal: () => formatDiscussionMinimal(ctx),
			};

			const formatter = formatters[opts.format] ?? (() => formatDiscussion(ctx, summary));
			...
			if (error instanceof AuthenticationError) {
				...
				console.error(`Discussion ${refStr} not found.`);
